...
for i in range(it_gen):
    ...
    Gscore = np.append(Gscore, [list(score(mlpclass(mlp1run(xtest, w1)), labeltest))], axis=0)
...
axe_x = np.linspace(1, it_train*it_gen, it_train*it_gen)
...
# affichage Scores:
axe_x = np.linspace(1, it_gen + 1, it_gen + 1)
